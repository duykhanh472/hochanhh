...
mod integration_tests {
    ...
    fn test_full_hochanh_workflow() {
        ...
        assert!(
            test_env_dir.join("hochanh.yml").exists(),
            "Thiếu hochanh.yml"
        );
        assert!(
            test_env_dir.join("src/sample-course/SUMMARY.md").exists(),
            "Thiếu SUMMARY.md"
        );
        assert!(
            test_env_dir.join("src/sample-course/lesson1.md").exists(),
            "Thiếu lesson1.md"
        );
        ...
        assert!(
            Path::new("site/index.html").exists(),
            "Không sinh ra trang chủ"
        );
        assert!(
            Path::new("site/css/style.css").exists(),
            "Không sinh ra file CSS"
        );
        assert!(
            Path::new("site/sample-course/lesson1.html").exists(),
            "Không dịch được bài học sang HTML"
        );
        ...
        assert!(
            http_response.contains("200 OK"),
            "Server không trả về trạng thái thành công 200 OK!"
        );
        assert!(
            http_response.contains("Danh sách khóa học"),
            "Nội dung trang chủ render bị sai!"
        );
        ...
        println!(
            "✨ [Chúc mừng] Toàn bộ hệ thống hochanh vượt qua bài kiểm tra tích hợp xuất sắc!"
        );
    }
}
